    ...
    public class ShoppingCart : IShoppingCart
    {
        ...
        public decimal GetTotal()
        {
            ...
            decimal? total = (from cartItems in storeDB.Carts
                              ...
                              cartItems.Product.ProductRate).Sum();
        ...
        public int CreateOrder(OrderEntity order)
        {
            ...
            foreach (var item in cartItems)
            {
                var orderDetail = new OrderDetail
                {
                    ...
                    UnitPrice = item.Product.ProductRate,
                    Quantity = item.Count
                };
                // Set the order total of the shopping cart
                orderTotal += (item.Count * item.Product.ProductRate);
        ...
        public void Payment(Payment payment)
        {
            ...
            {
                    dbConnect.pay.Add(payment);
                    dbConnect.SaveChanges();
            }
        }
        public IEnumerable<OrderDetail> GetOrderDetails(int productNumber)
        {
            using (DbConnect dbConnect = new DbConnect())
            {
                List<OrderDetail> products = dbConnect.OrderDetails.Include("Order").Include("Product").Where(x => x.OrderId == productNumber).ToList();
                return products;
            }
        }
    }
